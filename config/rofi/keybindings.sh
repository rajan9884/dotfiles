...
if command -v herdr >/dev/null 2>&1 && command -v menu-herdr-keybindings >/dev/null 2>&1; then
  menu-herdr-keybindings --print | rofi -dmenu -theme ~/.config/rofi/active-picker.rasi -p '⌨️  Keybindings' -i -lines 14
elif [ -x "$HOME/.config/hypr/scripts/keybinds-cheatsheet.sh" ]; then
  "$HOME/.config/hypr/scripts/keybinds-cheatsheet.sh" --print | rofi -dmenu -theme ~/.config/rofi/active-picker.rasi -p '⌨️  Keybindings' -i -lines 14
fi
